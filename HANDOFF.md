...
Next useful action: explain or disprove the observed Fibonacci/Lucas plateau pattern using small subset upper bounds and exact modular witnesses. The earlier interval-coverage-certificate question remains available. Follow Vance's next direction; no broad scan is authorized. Git history retains earlier stopping points.
