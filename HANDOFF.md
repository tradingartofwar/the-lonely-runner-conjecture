...
Next useful question: can a sharper local estimate use the positions of overlap intervals to handle small-gcd pairs? Begin with the 56:113 control and the tight (6,7,11,13) case. The original question remains partially answered: repeated arithmetic overlap is one sufficient mechanism, not a general resolution. Retain local concentration and exact equality points. Follow Vance's direction before expanding scope; no broad scan or independent reviewer contact is authorized. Interval-graph certificates are implemented; lattice certificates are not. Git history retains earlier stopping points.
