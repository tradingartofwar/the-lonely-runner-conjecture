...
Next useful action: group tight reference runners by normalized absolute relative speeds, accounting for permutation, sign symmetry, and common scaling. Use this to separate disguised copies of known distance curves from remaining differences in full-cycle coverage. Keep per-runner profiles, original runner counts, and equality endpoints. Follow Vance's next direction; no broad scan is authorized. Git history retains earlier stopping points.
