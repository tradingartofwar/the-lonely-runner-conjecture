...
Next conceptual question: what forces enough overlap when individual blocking durations could add up to complete coverage? At the fixed eight-runner threshold, each competitor blocks one-quarter of a full cycle. The local sum-of-durations estimate works for sufficiently fast groups of one, two, or three added runners, provided a fixed clear interval exists. With four, its leading clear-duration term vanishes; this estimate alone cannot settle coverage. That identifies a limit of the method, not a counterexample or a completed three-/four-variable result. Follow Vance's direction before expanding scope; no broad scan or independent reviewer contact is authorized. Interval-graph certificates are implemented; lattice certificates are not. Git history retains earlier stopping points.
