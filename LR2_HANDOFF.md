...
This narrows the existing near-doubling family by imposing A=q+8, while strengthening its certificate below the prior broader q>=242 cutoff. It is not a general-conjecture result, an all-reference result, or an externally reviewed proof. Next test +7 and +9 with their nonzero triple correction: can a constant-size affine-strip representation still supply useful slack uniformly in v>=q? Compare against simpler existing certificates and preserve failures; further cutoff polishing is not the objective.
