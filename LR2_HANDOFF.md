...
**Latest continuation — four blockers and cycle corrections:** [FOUR_BLOCKER_CYCLE_CORRECTIONS.md](notes/FOUR_BLOCKER_CYCLE_CORRECTIONS.md) compares the seven existing overlap-placement cases and the original 6/7/11/16 tree failure, keeping core 1/4/5 and J=`[9/32,3/8]`. All six pair overlaps are positive in each of the five fast cases. Compatibility exclusions improve the best tree in five of eight inputs. Four-way blocking of positive duration in 56/64/72/112 and 309/320/328/619 forces every graph satisfying the active-forest condition to be a global forest. This limits that certificate class, not all possible pair-data inequalities.
...
This is bounded analysis of existing examples, not new family coverage or a uniform algorithm: computing the moments still enumerates speed-dependent interval lists. Next ask for a speed-derived upper bound on the intersection of a selected cycle, starting with the existing near-doubling family and comparing against its simpler one-pair certificate. Keep equality and useful window selection separate. General selection and independent review remain open.
