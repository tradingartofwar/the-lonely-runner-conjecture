...
The next bounded question is whether speed arithmetic can select a small collection of occurrence incompatibilities sufficient for a useful certificate **without reconstructing every interval**. The complete occurrence-forest construction explains existing schedules; it does not force positive slack or solve core/window selection. Earlier graph-level pair-data limitations remain valid because the new representation contains additional occurrence information.
