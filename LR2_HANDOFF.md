...
Next test the certificate mechanism with a second variable speed, retaining the earlier 16/23 cooperative-cover example as a required control. TWO_VARIABLE_SPEEDS.md already covers existence for that family; the question is which sparse compatibility information and window choices carry over, not whether to repeat the old existence investigation. Arbitrary-speed core/window selection is still open.
