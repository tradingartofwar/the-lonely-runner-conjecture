...
**Continuation, September 24:** [OVERLAP_PLACEMENT.md](OVERLAP_PLACEMENT.md) uses the phase relation `113-2*56=1` to locate six evenly shrinking overlap windows. Endpoint durations and their overlap certify `U_J>=1223/911232>0`. A complete unreviewed argument extends this mechanism to `{0,1,4,5,q,2q+1,u,v}` for q>=310 and u,v>=q, all distinct. The tight case also contains the relation `13-2*6=1`, so the relation alone is insufficient for positive duration. A short full period is not necessary for a local certificate.
