...
September 24 continuation: [one variable-speed family](VARIABLE_SPEED_FAMILY.md) fixes `{1,4,5,6,7,11}` and lets the seventh relative speed range over every distinct positive integer. Divisibility and interval width reduce possible complete blockers to 8,16,24, each excluded exactly. A sharper modulo-7 observation gives an explicit witness for every speed: blocking both `1/8` and `17/56` forces a multiple of 56, which exits its blocking zone before the core's clear interval ends. This is a selected-reference argument with independent proof review outstanding, not a general noncoverage theorem.
