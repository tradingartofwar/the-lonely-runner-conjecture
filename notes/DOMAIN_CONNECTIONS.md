...
**Unbounded structured continuation:** [The uniform triangle certificate](UNIFORM_TRIANGLE_CERTIFICATE.md) turns that phase restriction into `T_(q,q+8,2q+1)=0` for all admissible q. Six fixed affine strips and exact periodic endpoint corrections compute the relevant pair durations without listing faster and faster schedules. The resulting proof candidate covers q>=28 with an arbitrary fourth speed v>=q in the stated family. This connects a model's retained relationship to a bounded number of computational operations. It still uses one actual clock; the auxiliary phase coordinate does not grant independently selectable runner positions. Nearby offsets +7 and +9 show where the exact exclusion stops.

**Distinction-audit direction:** [The September 25 audit](DISTINCTION_AUDIT_2026_09_25.md) pauses the next family extension and tests representation sufficiency against actual matched configurations. Three physical pairs preserve every joint blocking duration but change the topology of allowed times; one changes three intervals into four plus an isolated contact. Complete duration data determine total clear measure, but not that temporal/contact structure. A separate implication, B21 subset B6 union B7 on J, proves that some additional joint detail is irrelevant: replacing always-safe speed 2 by 21 changes overlap information while preserving the full allowed set. Closed safe-lap cells are constructed for both requested stress cases. The next suggested LTCM experiment uses a 16-state moment relaxation with selected speed-certified constraints, keeping abstract feasibility distinct from physical realizability. Formal information-theoretic synergy and a new physical field are not inferred.
