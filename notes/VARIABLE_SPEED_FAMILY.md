...
Continuation completed September 24: [two variable speeds](TWO_VARIABLE_SPEEDS.md) lets x,y vary while fixing `{1,4,5,6,7}`. Speeds 16 and 23 really do cover the entire old opening cooperatively. A new local duration bound handles all pairs with both speeds at least 34; certified intervals for 28 smaller speeds leave eight pairs, all with exact witnesses. The written argument covers the selected reference in this two-parameter integer family, with independent proof review still outstanding.
