...
**Continuation completed September 24:** [SPEED_RATIOS.md](SPEED_RATIOS.md) separates the reduced ratio's overlap fraction from the pair gcd's repetition period. It reproduces the known general pair formula, extends the local bound to arbitrary ratios, checks 3:1 and 3:2, and gives a conservative sufficient family with `gcd(A,B),u,v>=187`. The 56:113 control still has openings despite an inconclusive coarse bound. General four-speed coverage and independent review of our local-family argument remain unresolved.
