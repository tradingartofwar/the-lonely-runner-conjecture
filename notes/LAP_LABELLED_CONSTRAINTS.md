...
**Continuation completed:** [SPARSE_OVERLAP_SELECTION.md](SPARSE_OVERLAP_SELECTION.md) retains two yes/no triple-possibility tests and chooses an admissible weighted runner graph. A periodic duration formula avoids enumerating the variable runner's laps. An analytic tail plus 292 exact finite cases certifies this fixed window for every admissible integer replacement w, retaining four equality-only exceptions. Existence for the family was already covered in VARIABLE_SPEED_FAMILY.md; this is a compact certificate refinement. General selection for arbitrary speeds remains open.
